...
func AddDevTimer(devTmer *DevTimer) (int64, error) {
	var id int64
	...
	_, err = db.Exec(`insert into devicetimer( timerid,devid,on_off,turn_hour,turn_minute ) values($1,$2,$3,$4,$5);`, &id, &devTmer.DevID, &devTmer.OnOff, &devTmer.TurnHour, &devTmer.TurnMinute)
	if err != nil {
		log.Error(err)
		return 0, err
	}
	devTmer.TimerID = id
	devTimers.Add(devTmer)
